...
emp_data %>%
  ...
  mutate(
    print_data = map(data, function(x) {
      ...
      current %>%
        ...
        mutate(
          timeline = glue("{end} - {month.name[start_month]} {start_year}"),
...
format_bib("bib/conf.bib") %>%
  ...
  separate(
    eventdate,
    c("year", "month", "day", NA, NA, "end_day"),
    fill = "right",
    extra = "drop",
    convert = TRUE
  ) %>%
...
crn_pkgs <- db |>
  ...
  pull(Package)
git_pkgs <- c("atlas-aai/dlmr", "atlas-aai/rstandlm", "wjakethompson/wjake")
...
format_bib("bib/rpkg.bib") %>%
  ...
  ungroup() %>%
  mutate(
    year = case_when(count != 1 ~ "N/A", TRUE ~ as.character(year)),
...
awards <- awd_data %>%
  ...
  mutate(
    date = case_when(
      is.na(end_date) ~ as.character(start_date),
...
grants <- gnt_data %>%
  mutate(
    display_text = glue(
      "- **{role}:** *{title}*{ifelse(is.na(start), '', paste0(' (', start, '--', end, ')'))}. {sponsor}{ifelse(is.na(funding), '.', paste0('; $', prettyNum(funding, big.mark = ','), '.'))} {ifelse(is.na(pi), '', paste0('PI: ', pi, '.'))}"
    )
  ) %>%
  mutate(
    role = factor(
    ...
    status = factor(
      status,
      levels = c("Currently Funded", "Previously Funded", "Pending", "Unfunded")
    )
  ) |>
...
-   American Educational Research Association, Division H, Outstanding Publications
